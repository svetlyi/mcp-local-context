...
func TestLoadPromptsFromDirectory(t *testing.T) {
	...
	prompt1Content := `Prompt 1 description
	# Prompt 1
	...
	for _, prompt := range allPrompts {
		if prompt.Name == "prompt1" {
			foundPrompt1 = true
			assert.Equal(t, prompt1Content, prompt.Content, "Prompt1 content mismatch")
			assert.Equal(t, "Prompt 1 description", prompt.Description, "Prompt1 description should be extracted from first line")
		}
